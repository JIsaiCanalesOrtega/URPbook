...
class TextCleaner {
  ...
  static String _eliminarElementosEstructurales(String texto) {
    return texto
        ...
        .replaceAll(
          RegExp(r'^\s*\d{1,2}[/-]\d{1,2}[/-]\d{2,4}\s*$', multiLine: true),
          '',
        );
  }
  ...
  static String _unirLineasPartidasConservador(String texto) {
    ...
    texto = texto.replaceAllMapped(
      RegExp(
        r'\b(el|la|los|las|un|una|de|del|en|con|y|o|que|se|no|es|más|por|su|le|da|ha|su|al|lo|me|te|le|nos|os|les)\s*\n\s*([a-zA-ZáéíóúüñÁÉÍÓÚÜÑ])',
        caseSensitive: false,
      ),
      (m) => '${m[1]} ${m[2]}',
    );
